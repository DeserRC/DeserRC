<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=180&text=DeserRC&fontAlign=50&fontAlignY=38&color=gradient" />

<h1 align="center">Diego Cruz</h1>
<p align="center">
  Full Stack Developer with 8+ years of experience building robust, scalable solutions for the financial market and the game industry.
</p>

<p align="center">
  <a href="mailto:deserrc@gmail.com">
    <img src="https://img.shields.io/badge/Email-deserrc%40gmail.com-black?style=for-the-badge&logo=gmail" />
  </a>
  <a href="https://www.linkedin.com/in/deser/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-deser-black?style=for-the-badge&logo=linkedin" />
  </a>
  <a href="https://github.com/DeserRC" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-DeserRC-black?style=for-the-badge&logo=github" />
  </a>
</p>

---

## About me
I’m Diego Cruz, a Full Stack Developer with over 8 years of experience, specialized in building robust and scalable solutions for the financial market and the game industry. I work across a wide technical stack, including **Java, Kotlin, Go, Elixir, JavaScript, and TypeScript**, with frameworks such as **Spring, Angular, and React Native**.

My expertise ranges from **APIs and Microservices** development to integrating **relational databases** (SQLite, H2, MySQL, PostgreSQL, Oracle) and **NoSQL** solutions (MongoDB, Redis). I’m also proficient with messaging systems like **RabbitMQ** and **Kafka**, and CI/CD tooling including **GitHub Actions, GitLab CI/CD, and Jenkins**.

I have solid experience with **Docker, Kubernetes**, and cloud platforms such as **GCP, Azure, and AWS**, ensuring efficient deployments and reliable infrastructure. I hold a degree in **Systems Analysis and Development** from **Universidade Anhembi Morumbi**. I’m passionate about solving complex problems, combining technical depth with a practical mindset focused on delivering value, and I’m known for writing clean code and implementing innovative solutions.

---

## Stacks & tools
### Languages
![Java](https://img.shields.io/badge/Java-black?style=flat-square&logo=openjdk&logoColor=white)
![Kotlin](https://img.shields.io/badge/Kotlin-black?style=flat-square&logo=kotlin&logoColor=white)
![Go](https://img.shields.io/badge/Go-black?style=flat-square&logo=go&logoColor=white)
![Elixir](https://img.shields.io/badge/Elixir-black?style=flat-square&logo=elixir&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-black?style=flat-square&logo=javascript&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-black?style=flat-square&logo=typescript&logoColor=white)

### Frameworks
![Spring](https://img.shields.io/badge/Spring-black?style=flat-square&logo=spring&logoColor=white)
![Angular](https://img.shields.io/badge/Angular-black?style=flat-square&logo=angular&logoColor=white)
![React Native](https://img.shields.io/badge/React%20Native-black?style=flat-square&logo=react&logoColor=white)

### Databases
**Relational:** SQLite · H2 · MySQL · PostgreSQL · Oracle  
**NoSQL:** MongoDB · Redis

### Messaging
RabbitMQ · Kafka

### CI/CD
GitHub Actions · GitLab CI/CD · Jenkins

### DevOps & Cloud
Docker · Kubernetes · Google Cloud Platform (GCP) · Microsoft Azure · Amazon Web Services (AWS)

---

## Interests
- Scalable back-end systems, APIs, microservices
- Clean Architecture / DDD, maintainability, code quality
- Security, performance, observability
- Financial systems (high throughput / reliability)
- Game industry systems and interactive experiences
- Automation and developer productivity

---

## Projects
<p align="left">
  <a href="https://github.com/cefoler/databases" target="_blank" rel="noreferrer">
    <img width="49%" src="https://github-stats-extended.vercel.app/api/pin/?username=cefoler&repo=databases&theme=dark&description_lines_count=2&card_width=500&border_radius=10&cache_seconds=86400" />
  </a>
  <a href="https://github.com/cefoler/configuration" target="_blank" rel="noreferrer">
    <img width="49%" src="https://github-stats-extended.vercel.app/api/pin/?username=cefoler&repo=configuration&theme=dark&description_lines_count=2&card_width=500&border_radius=10&cache_seconds=86400" />
  </a>
</p>

<p align="left">
  <a href="https://github.com/cefoler/task" target="_blank" rel="noreferrer">
    <img width="49%" src="https://github-stats-extended.vercel.app/api/pin/?username=cefoler&repo=task&theme=dark&description_lines_count=2&card_width=500&border_radius=10&cache_seconds=86400" />
  </a>
  <a href="https://github.com/cefoler/celestium-protocol" target="_blank" rel="noreferrer">
    <img width="49%" src="https://github-stats-extended.vercel.app/api/pin/?username=cefoler&repo=celestium-protocol&theme=dark&description_lines_count=2&card_width=500&border_radius=10&cache_seconds=86400" />
  </a>
</p>

---

## GitHub Stats
<p align="left">
  <img width="49%" src="https://github-stats-extended.vercel.app/api?username=DeserRC&show_icons=true&theme=dark&hide_title=true&border_radius=10&cache_seconds=86400" />
  <img width="49%" src="https://github-stats-extended.vercel.app/api/top-langs/?username=DeserRC&layout=compact&theme=dark&hide_progress=true&border_radius=10&cache_seconds=86400" />
</p>
